...
export function fromDtoToEntity(userDto: UserDto): Partial<IUser> {
  const {
    username,
    password,
    first_name,
...
export function addUser(users: IUser[], user: IUser): IUser[] {
  return [...users, user];
}

export function removeUser(users: IUser[], user: IUser): IUser[] {
